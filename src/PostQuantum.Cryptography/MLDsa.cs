...
public sealed class MLDsaPrivateKey : IDisposable
{
    ...
    /// <see cref="MLDsaKey.ImportEncryptedPkcs8PrivateKey"/>.
    /// </summary>
    ...
    public byte[] ExportEncryptedPkcs8PrivateKey(ReadOnlySpan<char> password)
    {
        ...
        return _dsa.ExportEncryptedPkcs8PrivateKey(password, Pbe.Parameters);
    }
    ...
    /// <see cref="MLDsaKey.ImportEncryptedPrivateKeyFromPem"/>.
    /// </summary>
    /// <exception cref="ArgumentException"><paramref name="password"/> is empty.</exception>
    public string ExportEncryptedPkcs8PrivateKeyPem(ReadOnlySpan<char> password)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        Pbe.RequireNonEmptyPassword(password);
        return _dsa.ExportEncryptedPkcs8PrivateKeyPem(password, Pbe.Parameters);
    }
...
public static class MLDsaKey
{
    ...
    /// </summary>
    public static MLDsaPrivateKey ImportEncryptedPkcs8PrivateKey(ReadOnlySpan<char> password, ReadOnlySpan<byte> source)
        => new(MLDsa.ImportEncryptedPkcs8PrivateKey(password, source));
    ...
    /// the password is wrong.
    /// </summary>
    public static MLDsaPrivateKey ImportEncryptedPrivateKeyFromPem(ReadOnlySpan<char> password, ReadOnlySpan<char> pem)
    {
        PemLabels.RequireEncryptedPrivateKeyLabel(pem);
        return new(MLDsa.ImportFromEncryptedPem(pem, password));
    }
}
